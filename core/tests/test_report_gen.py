...
class TestReportGen(unittest.TestCase):
    ...
    def test_replay_pdf_generator_serverless(
        self,
    ...
    @patch("common.aws_service.s3_upload")
    def test_replay_pdf_generator_provisioned(
        self,
        mock_upload,
        mock_report,
        mock_analysis_summary,
    ...
    @patch("core.replay.report_util.Report")
    @patch("common.aws_service.s3_upload")
    def test_replay_pdf_generator_no_key(
        self,
        mock_upload,
        mock_report,
        mock_analysis_summary,
    ...
    @patch("core.replay.report_util.Report")
    @patch("common.aws_service.s3_upload")
    def test_replay_pdf_generator_client_error(
        self,
        mock_upload,
        mock_report,
        mock_analysis_summary,
    ...
    @patch(
        "common.aws_service.s3_upload",
        side_effect=ClientError({"Error": {"Code": "Error!", "Message": "oops"}}, "testing"),
    )
    def test_replay_pdf_generator_cannot_upload(
        self,
        mock_upload,
        mock_logger,
        mock_report,
    ...
    def test_get_raw_data_iterate(self, mock_read_data, mock_read_csv, mock_get_object):
        mock_tables = {
            1: {"sql": "somesql", "data": "somedata", "columns": "somecolumn", "data": None}
        }
        empty_df = pandas.DataFrame()
        mock_report = MagicMock(tables=mock_tables)
        mock_read_csv.return_value = empty_df
        report_gen.get_raw_data(mock_report, self.bucket, "some_replay_path", "somesql")
    ...
    def test_get_raw_data_error(self, mock_get_object):
        ...
        with self.assertRaises(SystemExit):
            report_gen.get_raw_data(self.report, self.bucket, "some_replay_path", "somesql")
    ...
    def test_initiate_connection_exception(
        ...
        with self.assertLogs("WorkloadReplicatorLogger", level="DEBUG") as mock_log:
            with self.assertRaises(SystemExit):
                with report_gen.initiate_connection(
    ...
    def test_unload_success(self, mock_initiate_connection, mock_listdir, mock_splitext):
        ...
        with patch("builtins.open", mocked_open):
            response = report_gen.unload(
                {"url": "someloc"}, "somerole", self.provisioned_cluster, self.user, self.replay
            )
    ...
    def test_create_presigned_url(self, mock_s3_generate_presigned_url):
        ...
        mock_s3_generate_presigned_url.assert_called_once_with(
            client_method="get_object", bucket_name="fakebucket", object_name="fakeobject"
        )

    @patch("common.aws_service.s3_generate_presigned_url")
    def test_create_presigned_url_fail(self, mock_s3_generate_presigned_url):
        mock_s3_generate_presigned_url.side_effect = ClientError(
