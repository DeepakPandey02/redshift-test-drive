    ...
    try:
        ...
        if action == "initiate":
            ...
            res = {
                "job_id": run_extract(
                    ...
                    simple_replay_extract_end_time=user_config.get(
                        "SIMPLE_REPLAY_EXTRACT_END_TIME"
                    ),
                    simple_replay_extract_overwrite_s3_path=user_config.get(
            ...
            res = {
                "status": get_serverless_identifier(
                    serverless_client,
                    user_config,
                    redshift_cluster_configuration,
                    cluster_identifier_prefix,
            ...
            if (
                user_config.get("PARAMETER_GROUP_CONFIG_S3_PATH") is None
                or user_config.get("PARAMETER_GROUP_CONFIG_S3_PATH") == "N/A"
            ):
            ...
            maintenance_track = (
                "N/A"
                if redshift_cluster_configuration.get("MAINTENANCE_TRACK") is None
                else redshift_cluster_configuration.get("MAINTENANCE_TRACK")
            )
            res = {
            ...
            res = {
                "status": create_serverless_namespace(
                    serverless_client,
            ...
            res = {
                "status": restore_serverless_snapshot(
                    serverless_client,
                    snapshot_id=user_config.get("SNAPSHOT_ID"),
                    redshift_iam_role=system_config.get("REDSHIFT_IAM_ROLE"),
                    snapshot_account_id=user_config.get("SNAPSHOT_ACCOUNT_ID"),
            ...
            res = {
                "status": create_serverless_workgroup(
                    serverless_client,
        ...
        ## Added to check for clusters in pending reboot after wlm change ##
            ...
            if endpoint_type.upper() == "SERVERLESS":
                ...
                res = {
                    "sql_id": run_sql_script_from_s3(
                        script_s3_path=user_config.get("DDL_AND_COPY_SCRIPT_S3_PATH"),
                        action=action,
            ...
            res = {
                "job_id": run_redshift_performance_test(
                    ...
                    cluster_identifier=cluster_identifier,
                    bucket_name=system_config.get("S3_BUCKET_NAME"),
                    performance_test_bootstrap_script=system_config.get(
                        "PERFORMANCE_TEST_BOOTSTRAP_SCRIPT"
                    ),
                    performance_test_python_script=system_config.get(
                        "PERFORMANCE_TEST_PYTHON_SCRIPT"
                    ),
                    sql_script_s3_path=user_config.get("SQL_SCRIPT_S3_PATH"),
                    number_of_parallel_sessions_list=user_config.get(
            ...
            res = {
                "job_id": run_replay(
                    ...
                    cluster_identifier=cluster_identifier,
                    extract_s3_path="s3://"
                    + system_config.get("S3_BUCKET_NAME")
                    + "/"
                    + system_config.get("EXTRACT_PREFIX")
                    + "/"
                    + what_if_timestamp
                    + "/",
            ...
            if endpoint_type.upper() == "SERVERLESS":
                ...
                script_s3_path = (
                    system_config.get("GATHER_COMPARISON_STATS_SCRIPT")
                    + "|"
                    + system_config.get("GATHER_COMPARISON_STATS_SERVERLESS_SCRIPT")
                )
                comparison_stats_s3_path = system_config.get("COMPARISON_STATS_S3_PATH")
            res = {
                "sql_id": gather_comparison_stats(
...
def validate_user_config(user_config, client, current_account_id):
    ...
    if snapshot_identifier != "N/A" and owner_account == current_account_id:
        owner_account = (
            ...
            if owner_account == "N/A"
            else owner_account
        )
        response = client.describe_cluster_snapshots(
            SnapshotIdentifier=snapshot_identifier, OwnerAccount=owner_account
        )
        ...
        get_cluster_params = client.describe_cluster_parameters(
            ParameterGroupName=cluster_param_grp
        )
        ...
        for param in get_cluster_params["Parameters"]:
            if param["ParameterName"] == "enable_user_activity_logging":
                if param["ParameterValue"] == "false":
                    raise ValueError(
                        "enable_user_activity_logging parameter is set to False, please set this to True"
                    )
                else:
        ...
        if (
            log_location != "N/A"
            and loggingStatusResponse.get("LogDestinationType") == "s3"
        ):
    ...
    for config in user_config["CONFIGURATIONS"]:
        if config["TYPE"] == "Provisioned" and config["NODE_TYPE"] not in [
            "ds2.xlarge",
            "ds2.8xlarge",
            "dc2.large",
            "dc2.8xlarge",
...
def gather_comparison_stats(
    ...
    external_schema_script,
    query_label_prefix,
    node_type,
    number_of_nodes,
    region,
    cluster_config_s3_path,
    endpoint_type,
    redshift_client,
):
    if endpoint_type.upper() == "SERVERLESS":
        ...
        try:
            print(
                "gather_comparison_stats >> Provisioned >> cluster_identifier >>"
            ...
            executeResponse = boto3.client("redshift-data").execute_statement(
                Database=db,
                DbUser=user,
                Sql="select * from sys_query_history limit 1",
        ...
        if sysQueryExists:
            ...
            comparison_stats_s3_path = comparison_stats_s3_path + "comparison_stats"
            return run_sql_script_from_s3(
                script_s3_path=script_s3_path_serverless,
                action=action,
                cluster_identifier=cluster_identifier,
                redshift_iam_role=redshift_iam_role,
                bucket_name=bucket_name,
            ...
            return run_sql_script_from_s3(
                script_s3_path=script_s3_path.split("|")[0],
                action=action,
                cluster_identifier=cluster_identifier,
                redshift_iam_role=redshift_iam_role,
                bucket_name=bucket_name,
...
def get_cluster_identifier(
    ...
    if (
        redshift_configurations.get("USER_FRIENDLY_NAME_SUFFIX") is None
        or redshift_configurations.get("USER_FRIENDLY_NAME_SUFFIX") == "N/A"
    ):
        if (
            redshift_configurations.get("WLM_CONFIG_S3_PATH") is None
            or redshift_configurations.get("WLM_CONFIG_S3_PATH") == "N/A"
        ):
            wlm_name = ""
            trim_wlm_name = 64
        else:
            ...
            wlm_name = (
                redshift_configurations.get("WLM_CONFIG_S3_PATH")
                .split("/")[-1]
                .replace(".json", "")
            )
        ...
        cluster_suffix = (
            redshift_configurations.get("NODE_TYPE")
            + "-"
            + redshift_configurations.get("NUMBER_OF_NODES")
        )
...
def check_pending_reboot_status(client, cluster_identifier):
    try:
        ...
        status = (
            ...
            + desc_paramgroup_status
        )
        if desc_paramgroup_status == "pending-reboot":
            print(
                "Cluster {} needs to be rebooted to apply the WLM config changes".format(
                    cluster_identifier
                )
            )
...
def update_parameter_group(
    ...
    if modified_parameter_group:
        client.modify_cluster_parameter_group(
            ParameterGroupName=parameter_group_name, Parameters=modified_parameter_group
        )
    return "Initiated"
    ...
    try:
        if snapshot_id is None or snapshot_id == "N/A":
            master_user_secret = json.loads(
                ...
                .get_secret_value(SecretId=secrets_manager_arn)
                .get("SecretString")
            )
        ...
        if msg == "ClusterAlreadyExists":
            ...
            source_node_type, source_number_of_nodes = get_source_cluster_config(
                client, snapshot_id
            )
            client.restore_from_cluster_snapshot(
                NumberOfNodes=source_number_of_nodes,
...
def run_sql_script_from_s3(
    ...
    result_cache="true",
    with_event=False,
    what_if_timestamp=None,
    comparison_stats_s3_path=None,
    comparison_results_s3_path=None,
    raw_comparison_results_s3_path=None,
    query_label_prefix=None,
    workgroup_name=None,
):
...
def run_sql(
    ...
    if endpoint_type.upper() == "PROVISIONED":
        ...
        while status != "AVAILABLE":
            ...
            time.sleep(300)
        res = boto3.client("redshift-data").execute_statement(
            Database=db, Sql=script, WithEvent=with_event, WorkgroupName=workgroup_name
        )
    query_id = res["Id"]
    done = False
    while not done:
...
def run_redshift_performance_test(
    client,
    cluster_identifier,
    bucket_name,
    performance_test_bootstrap_script,
    ...
    if sql_script_s3_path is None or sql_script_s3_path == "N/A":
        ...
        cluster_endpoint = (
            ...
            + ":"
            + str(desc.get("Endpoint").get("Port"))
            + "/"
            + db
        )
        response = boto3.client("batch").submit_job(
            jobName="AmazonRedshiftPerformanceTesting",
            jobQueue=job_queue,
            ...
            containerOverrides={
                ...
                "environment": [
                    ...
                    {"name": "PYTHON_SCRIPT", "value": performance_test_python_script},
                    {"name": "REDSHIFT_CLUSTER_ENDPOINT", "value": cluster_endpoint},
                    ...
                    {"name": "DISABLE_RESULT_CACHE", "value": disable_result_cache},
                    {"name": "DEFAULT_OUTPUT_LIMIT", "value": default_output_limit},
                    {"name": "MAX_NUMBER_OF_QUERIES", "value": max_number_of_queries},
                    {"name": "MAX_PARALLEL_SESSIONS", "value": max_parallel_sessions},
    ...
    if simple_replay_log_location is None or simple_replay_log_location == "N/A":
        ...
        response = boto3.client("batch").submit_job(
            ...
            containerOverrides={
                ...
                "environment": [
                    {"name": "BOOTSTRAP_SCRIPT", "value": extract_bootstrap_script},
                    {"name": "BUCKET_NAME", "value": bucket_name},
                    {
...
def run_replay(
    ...
    if simple_replay_log_location is None or simple_replay_log_location == "N/A":
        ...
        if endpoint_type.upper() == "PROVISIONED":
            ...
            response = serverless_client.get_workgroup(workgroupName=workgroupName)
            cluster_endpoint = (
                response.get("workgroup").get("endpoint").get("address")
                + ":"
                + str(response.get("workgroup").get("endpoint").get("port"))
                + "/"
                + db
            )
        ...
        response = boto3.client("batch").submit_job(
            ...
            containerOverrides={
                ...
                "environment": [
                    {"name": "BOOTSTRAP_SCRIPT", "value": replay_bootstrap_script},
                    {"name": "WHAT_IF_TIMESTAMP", "value": what_if_timestamp},
                    ...
                    {
                        "name": "SIMPLE_REPLAY_UNLOAD_STATEMENTS",
...
def create_serverless_namespace(
    ...
    try:
        ...
        serverless_client.create_namespace(
            adminUserPassword=master_user_password,
            adminUsername=master_user_name,
            dbName=database_name,
            defaultIamRoleArn=redshift_iam_role,
            iamRoles=[redshift_iam_role],
            namespaceName=namespace_name,
            tags=[{"key": "app_name", "value": "redshift_node_config_compare"}],
        )
        return "Initiated"
...
def restore_serverless_snapshot(
    serverless_client,
    snapshot_id,
    redshift_iam_role,
    snapshot_account_id,
...
def get_serverless_identifier(
    serverless_client, config, redshift_configurations, cluster_identifier_prefix
):
    if (
        redshift_configurations.get("USER_FRIENDLY_NAME_SUFFIX") is None
        or redshift_configurations.get("USER_FRIENDLY_NAME_SUFFIX") == "N/A"
        ...
        cluster_suffix = redshift_configurations.get("USER_FRIENDLY_NAME_SUFFIX")
    workgroup_name = (
        "workgroup-" + (cluster_identifier_prefix + "-" + cluster_suffix).lower()[0:63]
    )
    namespace_name = (
        "namespace-" + (cluster_identifier_prefix + "-" + cluster_suffix).lower()[0:63]
    )
